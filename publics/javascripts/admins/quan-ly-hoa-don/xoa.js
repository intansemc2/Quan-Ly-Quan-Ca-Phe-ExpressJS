...
function deleteHoaDonRowInTable(buttonDelete) {
    ...
    swal({
        title: `Bạn có chắc chắn muón xóa tài khoản có tên đăng nhập là "${hoadon.username}" không?`,
        text: `Không thể khôi phục dữ liệu sau khi xóa. Qúa trình sẽ xóa luôn các thông tin liên quan trong Cơ sở dữ liệu.`,
        icon: 'warning',
        buttons: {
...
function deleteHoaDonAJAX(hoadon) {
    $.ajax({ method: 'DELETE', url: '/api/tai-khoan', data: { idHoaDon: hoadon.idHoaDon } })
        .done(function (data, status, xhr) {
            if (data && data > 0) {
                swal('Đã xóa thành công !', { icon: 'success' });

                let tableRow = $('#tableQuanLyHoaDon').find(`.idHoaDon[data="${hoadon.idHoaDon}"]`).parents('tr');
