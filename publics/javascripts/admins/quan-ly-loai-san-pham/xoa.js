...
$(document).ready(function () {
    ...
    $("#deleteAll").click(function(){
        swal({
            title: `Bạn có chắc chắn muón muốn xóa tất cả loại sản phẩm không?`,
...
function deleteLoaiSanPhamRowInTable(buttonDelete) {
    ...
    swal({
        title: `Bạn có chắc chắn muón xóa loại sản phẩm có id loại sản phẩm là "${loaiSanPham.idLoaiSanPham}" không?`,
        text: `Không thể khôi phục dữ liệu sau khi xóa. Qúa trình sẽ xóa luôn các thông tin liên quan trong Cơ sở dữ liệu.`,
        icon: 'warning',
        buttons: {
...
function deleteLoaiSanPhamAJAX(loaiSanPham) {
    $.ajax({ method: 'DELETE', url: '/api/loai-san-pham', data: { idLoaiSanPham: loaiSanPham.idLoaiSanPham } })
        .done(function (data, status, xhr) {
            if (data && data > 0) {
                swal('Đã xóa thành công !', { icon: 'success' , timer: 1000});

                let tableRow = getRowInTable(loaiSanPham);
...
function deleteAllAJAX() {
    $.ajax({ method: 'DELETE', url: '/api/loai-san-pham', data: {} })
        .done(function (data, status, xhr) {
            if (data && data > 0) {
